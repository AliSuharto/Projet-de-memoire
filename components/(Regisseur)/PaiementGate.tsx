...
export default function PaiementGate() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      try {
        // En production, remplacez par votre vraie logique d'API
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // Simuler une session ouverte
        setSession({
          id: 1,
          nomSession: 'Session 2025-01',
          status: 'OUVERTE'
        });
      } catch (err) {
        setShowModal(true);
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-sm mx-4">
            ...
            <button
              onClick={() => alert('Redirection vers création de session')}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              ...
              <div className="lg:col-span-2 bg-white rounded-lg shadow-sm border p-6">
                ...
                <p className="text-gray-600">
                  Session active: <span className="font-semibold">{session.nomSession}</span>
                </p>
              </div>
