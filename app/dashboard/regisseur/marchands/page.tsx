  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6 lg:p-0 pt-20 md:pt-6">
      <div className="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 py-8">
        ...
        {!loading && !error && (
          <>
            {/* Compact Header with Search and Filter - STICKY - HIDDEN ON MOBILE */}
            <div className="hidden md:block bg-white rounded-xl shadow-lg p-4 mb-4 sticky top-30 md:top-0 z-30">
              <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                {/* Title */}
                <h1 className="text-lg font-bold text-slate-900 lg:mr-4">Liste des Marchands</h1>
                ...
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-10 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-200"
                    />
                    {searchTerm && (
                      <button
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              ...
              {/* Pagination */}
              {filteredAndSortedData.length > 0 && (
                <div className="px-4 md:px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4 w-full md:w-auto">
                    ...
                    <select
                      ...
                      className="border-2 border-slate-200 rounded-lg px-2 md:px-3 py-1.5 text-xs md:text-sm focus:outline-none focus:border-blue-500 transition-colors"
                    >
                  ...
                  <div className="flex items-center gap-1 md:gap-2 flex-wrap justify-center">
                    <button
                      ...
                      className="px-3 md:px-4 py-2 border-2 border-slate-200 rounded-lg text-xs md:text-sm font-medium text-slate-700 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      Précédent
                    </button>
...
function MarchandDetails({ marchand, onBack }: { marchand: Marchand; onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 pt-20 md:pt-6">
      {/* Sticky Back Button */}
      <div className="sticky top-20 md:top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-4">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-medium text-sm md:text-base"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl md:text-4xl font-bold text-slate-900 mb-2">Détails du Marchand</h1>
          <p className="text-sm md:text-base text-slate-600">Consultez les informations et l'historique des paiements</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8">
          <div className="border-l-4 border-blue-500 pl-4">
            ...
            <p className="text-lg md:text-xl font-bold text-slate-900 break-words">{marchand.nom}</p>
          </div>
          ...
          <div className="border-l-4 border-indigo-500 pl-4">
            ...
            <p className="text-sm md:text-base font-semibold text-slate-900 break-words">{getFullAddress()}</p>
          </div>
          ...
          <div className="border-l-4 border-purple-500 pl-4">
            ...
            <p className="text-sm md:text-base font-semibold text-slate-900 break-words">{marchand.activite}</p>
          </div>
          ...
          <div className="border-l-4 border-emerald-500 pl-4">
            ...
            <p className="text-sm md:text-base font-bold text-slate-900">MAR-{marchand.id}</p>
          </div>
          ...
          <div className="border-l-4 border-orange-500 pl-4">
            ...
            <div className="text-slate-800">
              <p className="text-xs md:text-sm break-words">{marchand.email || 'Non renseigné'}</p>
              {marchand.telephone && (
                <p className="text-xs md:text-sm">{marchand.telephone}</p>
              )}
            </div>
          </div>
        ...
        {/* Action Button */}
        <div className="mb-8">
          <button className="w-full md:w-auto px-6 md:px-8 py-3 md:py-4 bg-gradient-to-r from-emerald-600 to-green-600 text-white font-bold rounded-2xl hover:from-emerald-700 hover:to-green-700 transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-1 text-sm md:text-base">
        ...
        {/* Payments History */}
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="px-4 md:px-6 py-4 bg-gradient-to-r from-slate-50 to-slate-100 border-b-2 border-slate-200">
            <h2 className="text-xl md:text-2xl font-bold text-slate-900">Historique des Paiements</h2>
            {paiements.length > 0 && (
          ...
          {loading && (
            <div className="p-12 text-center">
              <div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-blue-500 border-t-transparent"></div>
              <p className="mt-4 text-slate-600 font-medium text-sm md:text-base">Chargement des paiements...</p>
            </div>
          )}
          ...
          {error && (
            <div className="p-6">
              <div className="bg-red-50 border-l-4 border-red-500 rounded-xl p-6">
                <p className="text-red-800 font-semibold text-sm md:text-base">Erreur : {error}</p>
              </div>
            </div>
          )}
            ...
            <>
              {paiements.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-600 font-medium text-sm md:text-base">Aucun paiement enregistré pour ce marchand</p>
                </div>
              ) : (
                  ...
                  {paiements.map((paiement) => (
                    <div key={paiement.id} className="border-b border-slate-100 last:border-b-0">
                      <button
                        ...
                        className="w-full px-4 md:px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors gap-4"
                      >
                        <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                          <div className="flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center">
                            <svg className="w-5 h-5 md:w-6 md:h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                          </div>
                          <div className="text-left min-w-0 flex-1">
                            <p className="font-semibold text-slate-900 text-sm md:text-base truncate">{paiement.motif}</p>
                            <p className="text-xs md:text-sm text-slate-500">{formatDateTime(paiement.datePaiement)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
                          <p className="text-base md:text-xl font-bold text-emerald-600">{formatMontant(paiement.montant)}</p>
                      ...
                      {expandedPaiement === paiement.id && (
                        <div className="px-4 md:px-6 pb-6 bg-slate-50">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-white rounded-xl p-4 shadow-sm">
                              <p className="text-xs font-semibold text-slate-500 mb-1">Référence :</p>
                              <p className="text-xs md:text-sm font-medium text-slate-900 break-words">{paiement.reference || `TXN-${paiement.id}`}</p>
                            </div>
                            <div className="bg-white rounded-xl p-4 shadow-sm">
                              <p className="text-xs font-semibold text-slate-500 mb-1">Numéro reçu :</p>
                              <p className="text-xs md:text-sm font-medium text-slate-900">{paiement.recuNumero || 'N/A'}</p>
                            </div>
                            <div className="bg-white rounded-xl p-4 shadow-sm">
                              <p className="text-xs font-semibold text-slate-500 mb-1">Date et Heure :</p>
                              <p className="text-xs md:text-sm font-medium text-slate-900">{formatDateTime(paiement.datePaiement)}</p>
                            </div>
                            <div className="bg-white rounded-xl p-4 shadow-sm">
                              <p className="text-xs font-semibold text-slate-500 mb-1">Régisseur / Percepteur :</p>
                              <p className="text-xs md:text-sm font-medium text-slate-900 break-words">{paiement.nomAgent || 'N/A'}</p>
                            </div>
                          </div>
                        </div>
                      )}
